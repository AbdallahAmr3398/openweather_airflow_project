...
def transform_load_data(task_instance):
    ...
    #load data_transformed to s3postgresql
def load_weather():
...
def save_joined_data_s3(task_instance):
    ...
    df = pd.DataFrame(data, columns = ['city', 'description', 'temperature_farenheit', 'feels_like_farenheit', 'minimun_temp_farenheit', 'maximum_temp_farenheit', 'pressure','humidity', 'wind_speed', 'time_of_record', 'sunrise_local_time', 'sunset_local_time', 'state', 'census_2023', 'land_area_sq_mile_2023'])
    # df.to_csv("joined_weather_data.csv", index=False)
    now = datetime.now()
    dt_string = now.strftime("%d%m%Y%H%M%S")
    dt_string = 'joined_weather_data_' + dt_string
    df.to_csv(f"s3://weather-buc/{dt_string}.csv", index=False)
default_args = {
    'owner': 'abdallah&tuqa',
...
with DAG('weather_dag_2',
        ...
        with TaskGroup(group_id = 'group_a', tooltip= "Extract_from_S3_and_weatherapi") as group_A:
            ...
            uploadS3_to_postgres  = PostgresOperator(
                ...
                postgres_conn_id = "postgres_conn",
                sql = "SELECT aws_s3.table_import_from_s3('city_look_up', '', '(format csv, DELIMITER '','', HEADER true)', 'weather-buc', 'us_city.csv', 'us-est-1');"
            )
            ...
            is_houston_weather_api_ready = HttpSensor(
                ...
                endpoint='/data/2.5/weather?q=Houston&appid=b191d58f07bed4f90000c0dbb24092a3'
            )
